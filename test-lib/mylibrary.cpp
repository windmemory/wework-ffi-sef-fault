...
int init (char* corpId, char* secret) {
  int ret = 0;
  string strcorpId = corpId;
  string strsecret = secret;
  const char* _corpId = strcorpId.c_str();
  const char* _secret = strsecret.c_str();

  printf("Initing... corpId: %s, secret: %s\n", _corpId, _secret);
  handle = dlopen("./libWeWorkFinanceSdk_C.so", RTLD_LAZY);
  if (!handle) {
    printf("load sdk so fail:%s\n", dlerror());
    return -1;
  }
  printf("sdk loaded\n");
  newsdk_t* newsdk_fn = (newsdk_t*)dlsym(handle, "NewSdk");
  sdk = newsdk_fn();
  Init_t* init_fn = (Init_t*)dlsym(handle, "Init");
  ret = init_fn(sdk, _corpId, _secret);
  return ret;
}

char* getChatData (int seq, int limit) {
  int ret = 0;
  uint64_t _seq = (uint64_t)seq;
  uint64_t _limit = (uint64_t)limit;
  printf("getChatData(%lu, %lu)\n", _seq, _limit);
  NewSlice_t* newslice_fn = (NewSlice_t*)dlsym(handle, "NewSlice");
  FreeSlice_t* freeslice_fn = (FreeSlice_t*)dlsym(handle, "FreeSlice");

  Slice_t* chatDatas = newslice_fn();
  printf("chatDatas created. %d\n", chatDatas->len);
  // getchatdata api
  GetChatData_t* getchatdata_fn = (GetChatData_t*)dlsym(handle, "GetChatData");
  printf("function got.\n");
  ret = getchatdata_fn(sdk, _seq, _limit, "", "", 10, chatDatas);
  printf("ret: %d\n", ret);
  // if (ret != 0) {
